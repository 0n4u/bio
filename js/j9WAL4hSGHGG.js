...
$(document).ready(() => {
    const links = [{
            name: 'Spotify',
            link: 'https://open.spotify.com/user/qh50ydvzvrec67e6x1fj7e5ig?si=bd7effef3f664c49',
        },
        {
            name: 'Discord',
            link: 'https://discord.gg/vRzgzk8wwH',
        },
        {
            name: 'Soundcloud',
            link: 'https://soundcloud.com/delay-0',
        },
    ];
    ...
    app.titleChanger(['0', '0m', '0ms', '0m', '0', '']);
});
...
$('html').on('contextmenu', (event) => {
    ...
    const kingvonpiracy = app.skippedIntro ? '' : 'kingvon-piracy';

    img.src = './images/e38.gif';
    img.width = 64;
    img.height = 64;
    img.alt = 'silence';
    img.style = `position: absolute; left: ${event.pageX}px; top: ${event.pageY}px; z-index: 10`;
    img.className = `kingvon ${kingvonpiracy}`;
...
$.getJSON(ipgeolocation, (data) => {
    writeLine(["Init connection <span style=\"font-size: 14px; color: #06d;\">@Server 192.124.135.239</span>", "Waiting for requests...  <span style=\"font-size: 14px; color: #06d;\">[unknown]</span>"], 30, () => {
        ...
        writeLine([`Access granted! <span style="font-size: 14px; color: #0f0;">[success]</span>`, `Welcome back, <i style="color: #0f0">${ip}</i> By the way, nice to see someone from ${country} here!`], 30, 500, () => {
            ...
            writeLine([`<i style="color: #F62459">0ms.</i>`], 120, 500, () => {
                timeouts.push(
                    setTimeout(() => {
                        if (app.skippedIntro) return;
